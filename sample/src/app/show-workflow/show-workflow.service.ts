...
export class ShowWorkflowService {
  private products  = []; 
  ...
  constructor(private httpService: HttpClient) { 
   ...
   get_workflow(id){
     ...
     const  params = new  HttpParams({fromString:  'processId='+id});
    headers.append('Access-Control-Allow-Headers', 'Content-Type');
    headers.append('Access-Control-Allow-Methods', 'GET');
    headers.append('Access-Control-Allow-Origin', '*'); 
    headers.append("Referrer-Policy", "no-referrer");
    return this.httpService.get('http://localhost:8082/hbpm/process/{processId}/workflow/retrieve?',{params,headers}).map(res => res as Proces[] || []);
}
